...
import {
  Star, Phone, MessageCircle, CalendarDays, ChevronRight,
  CheckCircle, Globe, Award, Clock, Users, Gem, Sparkles,
  ArrowRight, Shield
} from 'lucide-react';

import SEO from '../components/common/SEO';
import Schema, { LOCAL_BUSINESS_SCHEMA, buildFAQSchema } from '../components/common/Schema';
...
const FEATURED_ARTICLES  = ARTICLES.filter((a) => a.featured).concat(ARTICLES.filter((a) => !a.featured)).slice(0, 3);
...
function HeroSection() {
  return (
    <section
      ...
      style={{
        background: 'linear-gradient(160deg, #FBF5EC 0%, #FFF8F0 40%, #FDF4E8 100%)',
        minHeight: '85vh',
        display: 'flex',
        alignItems: 'center',
      }}
    >
      ...
      <div
        ...
        style={{ width: '55vw', height: '55vw', top: '-20%', right: '-15%', opacity: 0.5 }}
      />
      <div
        className="aurora-orb aurora-orb-maroon"
        style={{ width: '40vw', height: '40vw', bottom: '-10%', left: '-10%', opacity: 0.35 }}
      />
      <div
        className="aurora-orb aurora-orb-gold"
        style={{ width: '25vw', height: '25vw', top: '40%', left: '30%', opacity: 0.2 }}
      />
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-28 lg:py-36">
        <div className="max-w-3xl">
          ...
          >
            Personal Vedic astrology consultations with Dr. Gurudeva — trusted by thousands of families across the USA, India and worldwide.
          </motion.p>
          ...
          >
            <Link to="/book" className="btn-gold btn-shine !px-8 !py-4 !text-base w-full sm:w-auto justify-center">
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-maroon-200 text-sm mt-1 leading-relaxed max-w-lg">
                Consult Dr. Gurudeva directly at <strong className="text-white">732-448-0667</strong> for your first horoscope/Kundli reading — free of charge.
                Limited to one person per family. Vedic Astrology is a family heritage to Dr. Gurudeva.
              </p>
              <p className="text-maroon-300 text-xs mt-1.5">
                Available for: India · Bangladesh · Pakistan · Nepal · Sri Lanka · EST timezone USA
              </p>
            </div>
          </div>
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 lg:flex-shrink-0">
            <p className="text-maroon-200 text-xs leading-relaxed max-w-xs">
              Be ready with Date of Birth, time and place of birth when you call.
              In-person consultations and email/website readings are paid services.
            </p>
            <a
...
function ConsultationOptions() {
  const options = [
    { icon: Phone,         label: 'Phone Consultation', desc: 'Call from anywhere',  href: `tel:${CONTACT_INFO.phone}`, external: true },
    { icon: MessageCircle, label: 'WhatsApp',           desc: 'Chat & schedule',     href: `https://wa.me/${CONTACT_INFO.whatsapp}`, external: true },
    { icon: CalendarDays,  label: 'Book Online',        desc: 'Secure & instant',    href: '/book', external: false },
    { icon: Globe,         label: 'Zoom / Video',       desc: 'Face-to-face online', href: '/book?type=video', external: false },
  ];

  return (
    <div
      style={{
        background: 'linear-gradient(to right, #FFF9F0, #FFFFFF, #FFF9F0)',
        borderTop: '1px solid rgba(139,26,26,0.08)',
        borderBottom: '1px solid rgba(139,26,26,0.08)',
        boxShadow: '0 4px 24px rgba(139,26,26,0.04)',
      }}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 md:grid-cols-4" style={{ borderLeft: '1px solid rgba(139,26,26,0.06)' }}>
          {options.map(({ icon: Icon, label, desc, href, external }) => {
            const Wrapper = external ? 'a' : Link;
            const extra   = external ? { href, target: href.startsWith('http') ? '_blank' : undefined, rel: 'noreferrer' } : { to: href };
            return (
              <Wrapper
                key={label}
                {...extra}
                className="flex items-center gap-3 px-3 py-4 sm:px-5 sm:py-5 transition-all duration-200 group hover:bg-amber-50/70"
                style={{ borderRight: '1px solid rgba(139,26,26,0.06)' }}
              >
                <div
                  className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 transition-all duration-300 group-hover:scale-110"
                  style={{
                    background: 'linear-gradient(135deg, rgba(201,150,12,0.12), rgba(139,26,26,0.08))',
                ...
                <div>
                  <p className="text-xs sm:text-sm font-semibold text-maroon-900 group-hover:text-maroon-600 transition-colors leading-tight">{label}</p>
                  <p className="text-xs text-stone-400 mt-0.5 hidden sm:block">{desc}</p>
                </div>
              </Wrapper>
            );
          })}
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {reasons.map(({ icon: Icon, title, desc }, i) => (
          ...
          >
            ...
            <div>
              <h3 className="font-serif text-base font-semibold text-maroon-900 mb-1">{title}</h3>
...
function HowItWorksSection() {
  const steps = [
    { num: '01', title: 'Book Your Session',      desc: 'Choose a consultation type and select a convenient date and time.' },
    { num: '02', title: 'Provide Birth Details',  desc: 'Share your date, place and time of birth. Dr. Gurudeva prepares before your session.' },
    { num: '03', title: 'Receive Your Reading',   desc: 'A personal, in-depth consultation — phone, video or in-person.' },
    { num: '04', title: 'Follow Up with Clarity', desc: 'Written summary, remedy guidance and WhatsApp follow-up for 2–6 weeks.' },
  ];
    ...
    <SectionWrapper variant="aurora">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8 relative">
        ...
        <div
          ...
          style={{ background: 'linear-gradient(90deg, transparent, rgba(201,150,12,0.3) 20%, rgba(201,150,12,0.3) 80%, transparent)' }}
        />
        {steps.map(({ num, title, desc }, i) => (
          ...
          >
            <div
              ...
              style={{
                background: 'linear-gradient(135deg, rgba(139,26,26,0.1), rgba(201,150,12,0.12))',
                border: '1px solid rgba(201,150,12,0.25)',
                boxShadow: '0 2px 12px rgba(139,26,26,0.1)',
              }}
            >
              <span className="font-serif gradient-text-gold text-xl font-bold">{num}</span>
            </div>
            <h3 className="font-serif text-maroon-900 font-semibold text-base mb-2">{title}</h3>
            <p className="text-stone-500 text-sm leading-relaxed">{desc}</p>
          </motion.div>
        ))}
      </div>
...
function LocationSection() {
  const links = [
    ...
    { label: 'South Indian Astrology USA',   path: '/languages/south-indian-astrology' },
  ];

  return (
    <div style={{ background: '#F5EDE0', borderTop: '1px solid rgba(139,26,26,0.1)' }} className="py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
