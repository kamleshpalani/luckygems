    ...
    <>
      ...
      <section className="bg-gradient-to-br from-stone-800 to-stone-900 text-white py-8 md:py-16 lg:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Breadcrumbs crumbs={[{ label: page.heroHeading, path: `/locations/${slug}` }]} />
          <motion.div
          ...
          >
            ...
            <p className="text-stone-300 text-lg leading-relaxed mb-6">{page.heroParagraph || page.regionDesc}</p>
      ...
      <SectionWrapper variant="white">
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          <div>
            ...
            <h2 className="section-title mb-2">Vedic Astrology for {page.heroHeading.replace('Indian Astrologer in ', '').replace('Indian Astrologer – ', '')}</h2>
            <div className="gold-divider" />
            <p className="text-stone-600 leading-relaxed mt-4">{page.regionDesc}</p>
            {page.additionalDesc && (
          ...
          <div className="space-y-4">
            <div className="card-gold p-6">
              <h3 className="font-serif text-lg font-semibold text-stone-900 mb-4">Why Clients in This Region Choose Dr. Gurudeva</h3>
              <ul className="space-y-2.5">
                {['Over 25 years of experience in Vedic astrology', '100% online — available from anywhere', 'Consultations in English, Tamil, Telugu, Kannada, Hindi', 'Detailed written reports available', 'Trusted by thousands of clients worldwide'].map((f) => (
      ...
      {page.cities && page.cities.length > 0 && (
        <SectionWrapper variant="ivory">
          <SectionHeader label="Serving Cities" title="Clients From Across the Region" subtitle="Dr. Gurudeva has served clients from all major cities and communities in this region." />
          <div className="flex flex-wrap gap-2 justify-center">
            {page.cities.map((city) => (
              <span key={city} className="px-3 py-1.5 bg-white border border-stone-200 rounded-full text-stone-700 text-sm shadow-sm">
                {city}
              </span>
            ))}
          </div>
      ...
      <SectionWrapper variant="white">
        <div className="text-center max-w-xl mx-auto">
          <SectionHeader label="Available Services" title="What Dr. Gurudeva Offers" subtitle="Full range of Vedic astrology services available for clients in this region." />
