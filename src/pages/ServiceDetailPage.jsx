...
import { CheckCircle, Users, Gift, MessageCircle, AlertCircle, FileText, Calendar } from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <div className="inline-flex items-center gap-2 bg-white/10 border border-white/20 rounded-full px-3 py-1 mb-5">
              ...
              <span className="text-xs text-gold-200 font-medium">{service.category}</span>
            </div>
        ...
        <div className="max-w-5xl mx-auto grid lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 space-y-12">
            ...
            <div>
              <div className="inline-flex items-center gap-2 mb-3">
                ...
                <h2 className="font-serif text-xl font-semibold text-stone-900">Who This Service Is For</h2>
              </div>
              <ul className="space-y-3">
                {service.forWhom.map((item) => (
                  <li key={item} className="flex items-start gap-3 text-stone-600 text-sm">
                    <CheckCircle size={15} className="text-maroon-400 mt-0.5 flex-shrink-0" />
                    {item}
                  </li>
                ))}
              </ul>
            ...
            <div>
              <div className="inline-flex items-center gap-2 mb-3">
                ...
                <h2 className="font-serif text-xl font-semibold text-stone-900">What You Will Receive</h2>
              </div>
              <ul className="space-y-3">
            ...
            {service.notes?.length > 0 && (
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 space-y-3">
                <div className="inline-flex items-center gap-2 mb-1">
                  ...
                  <h2 className="font-serif text-base font-semibold text-amber-900">Important Notes</h2>
                </div>
                {service.notes.map((note, i) => (
                  <p key={i} className="text-amber-800 text-sm leading-relaxed">
                    <span className="font-semibold">Note {i + 1}: </span>{note}
                  </p>
                ))}
              </div>
            )}
            ...
            {service.questionCategories?.length > 0 && (
              <div>
                <div className="inline-flex items-center gap-2 mb-3">
                  ...
                  <h2 className="font-serif text-xl font-semibold text-stone-900">Sample Questions</h2>
                </div>
                <p className="text-stone-500 text-sm mb-5">
                  Choose from the list below or write your own question. Each answer is{' '}
                  <strong className="text-maroon-600">$11</strong>. You will also receive a free 50-page horoscope report.
                </p>
                <div className="space-y-4">
                  {service.questionCategories.map((cat) => (
                    <div key={cat.category} className="bg-stone-50 border border-stone-100 rounded-xl p-4">
                      <h3 className="font-semibold text-maroon-700 text-sm mb-2.5">{cat.category}</h3>
                      <ul className="space-y-2">
                        {cat.questions.map((q) => (
                          <li key={q} className="flex items-start justify-between gap-3 text-stone-600 text-sm">
                            <span className="flex items-start gap-2">
                              <CheckCircle size={13} className="text-gold-500 mt-0.5 flex-shrink-0" />
                              {q}
                            </span>
                            <span className="font-semibold text-maroon-600 text-xs whitespace-nowrap ml-2">$11</span>
                          </li>
                        ))}
                      </ul>
                    </div>
            ...
            {service.muhurthamTypes?.length > 0 && (
              <div>
                <div className="inline-flex items-center gap-2 mb-3">
                  ...
                  <h2 className="font-serif text-xl font-semibold text-stone-900">Available Muhurthams</h2>
                </div>
                <p className="text-stone-500 text-sm mb-4">
                  Please mention the ceremony venue city and the date range you need in additional comments when ordering.
                </p>
                <div className="rounded-xl border border-stone-200 overflow-hidden">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-stone-50 border-b-2 border-gold-300">
                        <th className="text-left px-4 py-2.5 font-semibold text-stone-700">Event</th>
                        <th className="text-right px-4 py-2.5 font-semibold text-stone-700">Price</th>
                      </tr>
                    </thead>
                    <tbody>
                      {service.muhurthamTypes.map(({ event, price }) => (
                        <tr key={event} className="border-b border-stone-100 hover:bg-amber-50 transition-colors">
                          <td className="px-4 py-2.5 text-stone-700">{event}</td>
                          <td className="px-4 py-2.5 text-right text-maroon-700 font-semibold">{price}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            {service.maritalQuestions?.length > 0 && (
              <div>
                <div className="inline-flex items-center gap-2 mb-3">
                  <Users size={16} className="text-maroon-500" />
                  <h2 className="font-serif text-xl font-semibold text-stone-900">Questions Covered in This Service</h2>
                </div>
                <ul className="space-y-2.5">
                  {service.maritalQuestions.map((q) => (
                    <li key={q} className="flex items-start gap-3 text-stone-600 text-sm">
            ...
            {service.sampleReport && (
              <div className="border border-gold-200 rounded-xl overflow-hidden">
                <div className="bg-gradient-to-r from-maroon-800 to-maroon-700 px-5 py-3 flex items-center gap-2">
                  ...
                  <h2 className="font-serif text-base font-semibold text-white">{service.sampleReport.label}</h2>
                </div>
                <div className="p-5 space-y-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-stone-900 text-sm">{service.sampleReport.name}</p>
                      <p className="text-stone-500 text-xs">{service.sampleReport.analyst}</p>
                    </div>
                  </div>
            ...
            <div>
              <h2 className="font-serif text-xl font-semibold text-stone-900 mb-3">Consultation Format</h2>
              <div className="flex flex-wrap gap-2">
                {service.formats.map((fmt) => (
                  <span key={fmt} className="badge px-3 py-1.5">{fmt}</span>
                ))}
              </div>
            </div>
            ...
            {service.relatedSlugs?.length > 0 && (
              <RelatedServices slugs={service.relatedSlugs} currentSlug={slug} />
            )}
          </div>
          ...
          <div className="space-y-5">
            ...
            <div className="card p-5 text-center space-y-3">
              ...
              <Link to="/book" className="btn-outline w-full justify-center !text-sm">
                Book Consultation
              </Link>
            </div>
          </div>
