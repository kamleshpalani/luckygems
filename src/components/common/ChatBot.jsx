...
import {
  ...
  Phone, Calendar, Sparkles, ChevronRight,
} from 'lucide-react';
...
const CONCERN_OPTIONS = [
  { id: 'horoscope',  label: '🔮 Horoscope & Birth Chart', slug: 'horoscope-reading' },
  { id: 'career',     label: '💼 Career & Business',       slug: 'career-astrology' },
  { id: 'marriage',   label: '💑 Marriage & Compatibility', slug: 'marriage-compatibility' },
  ...
  { id: 'question',   label: '❓ Ask a Specific Question', slug: 'ask-a-question' },
];

function detectIntent(text) {
  const t = text.toLowerCase().trim();

  if (/^(hi\b|hello|hey\b|namaste|hii|helo|good morning|good evening|good afternoon|greet)/.test(t))
    return 'greeting';
  if (/\b(book|appointment|schedule|book a consult)\b/.test(t)) return 'book';
  if (/\b(phone|call|contact|number|reach|whatsapp|wa\.me)\b/.test(t)) return 'contact';
  if (/\b(hour|timing|available|open|when can|office hour)\b/.test(t)) return 'hours';
  if (/\b(price|cost|fee|how much|charge|\$|rate)\b/.test(t)) return 'pricing';
  if (/\bfree\b/.test(t)) return 'free';
  if (/\b(payment|pay|paypal|credit card|wire|bank)\b/.test(t)) return 'payment';
  if (/\b(horoscope|birth chart|kundli|rashi|nakshatra|jathakam|janam patri|natal chart)\b/.test(t))
    return 'horoscope';
  if (/\b(career|job|profession|work|business|promotion|job change)\b/.test(t)) return 'career';
  if (/\b(marriage|kundali match|compatib|wedding|bride|groom|manglik dosha)\b/.test(t))
    return 'marriage';
  if (/\b(gemstone|gem\b|stone|ruby|sapphire|neelam|manik|pearl|coral|emerald|navratna|blue sapphire|hessonite)\b/.test(t))
    return 'gemstone';
  if (/\b(remedy|remedies|dosha|kaal sarp|sade sati|black magic|spiritual healing|pooja|homa|yantra|vastu)\b/.test(t))
    return 'remedy';
  if (/\b(muhurtham|muhurat|auspicious timing|griha pravesh|cesarean|c.section)\b/.test(t))
    return 'muhurtham';
  if (/\b(language|telugu|tamil|hindi|kannada|malayalam)\b/.test(t)) return 'language';
  if (/\b(numerology|lucky number)\b/.test(t)) return 'numerology';
  if (/\b(about|who is|experience|dr.*gurudeva|gurudeva|doctor|25 year)\b/.test(t)) return 'about';
  if (/\b(recommend|suggest|which service|what service|best service|help me choose|not sure|dont know|don.t know)\b/.test(t))
    return 'recommend';
  if (/\b(email|online report|email service|email reading)\b/.test(t)) return 'email';

  // Fuzzy FAQ match
  const matched = FLAT_FAQS.find((faq) => {
    const words = faq.q.toLowerCase().split(/\s+/).filter((w) => w.length > 4);
...
function getResponse(intent) {
  if (typeof intent === 'object' && intent.type === 'faq') {
    return { text: intent.item.a, quickReplies: ['book', 'contact', 'recommend'] };
  }
  switch (intent) {
    case 'greeting':
      return {
    ...
    case 'horoscope':
      return {
        ...
        action: { label: 'View Horoscope Service', to: '/services/horoscope-reading' },
    ...
    case 'career':
      return {
        ...
        action: { label: 'View Career Service', to: '/services/career-astrology' },
    ...
    case 'marriage':
      return {
        ...
        action: { label: 'View Compatibility Service', to: '/services/marriage-compatibility' },
        quickReplies: ['book', 'pricing'],
      };
    case 'gemstone':
      return {
    ...
    case 'numerology':
      return {
        ...
        action: { label: 'View Numerology Service', to: '/services/numerology' },
        quickReplies: ['book', 'pricing'],
      };
    case 'about':
      return {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-2">
        {/* Bubble */}
        <div className="bg-white border border-stone-200 rounded-2xl rounded-tl-sm px-4 py-3 shadow-sm text-stone-700 text-sm leading-relaxed">
          {formatted}
        </div>
            ...
            {msg.quickReplies.map((qr) => {
              ...
              return (
                <button
                  ...
                  className="text-xs border border-maroon-300 hover:border-maroon-500 text-maroon-700 hover:text-maroon-800 bg-white hover:bg-maroon-50 px-3 py-1.5 rounded-full transition-colors"
                >
                  {def.label}
                </button>
              );
        ...
        {msg.type === 'recommend' && (
          <div className="flex flex-col gap-1.5 mt-0.5">
            {CONCERN_OPTIONS.map((opt) => (
              <button
                key={opt.id}
                onClick={() => onQuickReply('concern_' + opt.id, opt.label, opt.slug)}
                className="text-left text-xs border border-stone-200 hover:border-maroon-400 text-stone-700 hover:text-maroon-800 bg-white hover:bg-maroon-50 px-3 py-2 rounded-xl transition-colors flex items-center gap-2"
              >
                {opt.label}
              </button>
            ))}
...
function LeadForm({ stage, onSubmit, onSkip }) {
  ...
  const placeholder = stage === 'lead_name' ? 'Your name…' : 'Your email address…';
  ...
  return (
    <div className="flex gap-2">
      <input
        ...
        className="flex-1 text-sm border border-stone-300 rounded-xl px-3 py-2 focus:outline-none focus:border-maroon-400 bg-white"
...
export default function ChatBot() {
  ...
  const pushBot = useCallback((payload, delay = 600) => {
    setTyping(true);
    setTimeout(() => {
      setTyping(false);
  ...
  const handleSend = useCallback((text) => {
    ...
    if (stage === 'lead_name') {
      ...
      setStage('lead_email');
      pushBot({ text: `Thanks, ${trimmed}! 😊 What's the best email address to reach you?` });
      return;
    }
    if (stage === 'lead_email') {
      pushUser(trimmed);
  ...
  const handleQuickReply = useCallback((intent, label, concernSlug) => {
    pushUser(label);
        ...
        {open && (
          <>
            ...
            <motion.div
              ...
              className={[
                'fixed z-50 flex flex-col bg-stone-50 shadow-2xl overflow-hidden',
            ...
            >
              ...
              <div className="bg-gradient-to-r from-maroon-800 to-maroon-700 px-4 py-3.5 flex items-center gap-3 flex-shrink-0">
                ...
                <div className="flex-1">
                  <p className="font-semibold text-white text-sm leading-snug">Dr. Gurudeva's Assistant</p>
              ...
              <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 scroll-smooth">
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          <div className="flex gap-2">
                            <button
                              ...
                              className="text-xs border border-maroon-300 hover:border-maroon-500 text-maroon-700 hover:bg-maroon-50 bg-white px-3 py-1.5 rounded-full transition-colors"
                            >
                              ✅ Yes, please
                            </button>
                            <button
                              onClick={() => handleQuickReply('book', '📅 Book Consultation')}
                              className="text-xs border border-stone-200 text-stone-600 hover:bg-stone-100 bg-white px-3 py-1.5 rounded-full transition-colors"
                            >
                              📅 Book directly
                            </button>
                          </div>
                ...
                {typing && (
                  <div className="flex items-center gap-2.5">
                    ...
                    <div className="bg-white border border-stone-200 rounded-2xl rounded-tl-sm shadow-sm">
                      <TypingDots />
                    </div>
                  </div>
                )}
              ...
              {/* Input bar */}
              <div className="flex-shrink-0 border-t border-stone-200 bg-white px-3 py-3">
                {(stage === 'lead_name' || stage === 'lead_email') ? (
                  ...
                  >
                    <input
                      ...
                      className="flex-1 text-sm border border-stone-300 rounded-xl px-3 py-2 focus:outline-none focus:border-maroon-400 bg-white placeholder:text-stone-400"
                    />
                    <button
                      type="submit"
                      disabled={!input.trim()}
                ...
                <p className="text-center text-stone-400 text-[10px] mt-2">
                  Powered by LuckyGems · <a href={`tel:${CONTACT_INFO.phone}`} className="underline hover:text-maroon-500">{CONTACT_INFO.phoneDisplay}</a>
                </p>
              </div>
            </motion.div>
          </>
