  ...
  if (variant === "maroon") {
    return (
      <section
        ...
        style={{
          background:
            "linear-gradient(135deg, #6F1414 0%, #8B1A1A 40%, #7a1616 70%, #5a1010 100%)",
        }}
      >
        {/* Aurora orbs */}
        <div
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(90deg, transparent, rgba(201,150,12,0.5) 30%, rgba(255,255,255,0.2) 50%, rgba(201,150,12,0.5) 70%, transparent)",
          }}
        />
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          >
            ...
            <p className="text-maroon-100/80 text-base md:text-lg mb-10 max-w-2xl mx-auto">
              {subtext}
            </p>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link
                to={primaryCTA.path}
                className={`${styles.primary} !px-10 !py-4 !text-base w-full sm:w-auto justify-center`}
              >
              ...
              {secondaryCTA ? (
                <Link
                  ...
                  className={`inline-flex items-center gap-2 px-10 py-4 rounded-xl font-semibold text-base transition-all ${styles.secondary}`}
                >
                  {secondaryCTA.label}
                </Link>
              ) : (
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(90deg, transparent, rgba(201,150,12,0.4) 40%, rgba(201,150,12,0.4) 60%, transparent)",
          }}
        />
      </section>
    );
  ...
  return (
    <section className={`py-16 md:py-20 ${styles.wrapper}`}>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        >
          <h2
            className={`font-serif text-3xl md:text-4xl font-semibold mb-4 ${styles.heading}`}
          >
            {heading}
          </h2>
          <p
            className={`text-base md:text-lg mb-8 max-w-2xl mx-auto ${styles.sub}`}
          >
            {subtext}
          </p>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link
              to={primaryCTA.path}
              className={`${styles.primary} !px-8 !py-3.5`}
            >
            ...
            {secondaryCTA ? (
              <Link
                to={secondaryCTA.path}
                className={`inline-flex items-center gap-2 px-8 py-3.5 rounded-lg font-medium text-sm transition-all ${styles.secondary}`}
              >
                {secondaryCTA.label}
              </Link>
            ) : (
